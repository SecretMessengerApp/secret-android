...
class FavoriteFragment(override val onEmojiChangeListener:OnEmojiChangeListener) extends BaseGifFragment(onEmojiChangeListener) {

  override def getData: Future[util.List[GifSavedItem]] = Future {
    GifSavedDaoHelper.getSavedGIFs(SpUtils.getUserId(getContext),false)
