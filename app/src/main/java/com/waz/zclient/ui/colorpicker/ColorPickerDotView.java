...
public class ColorPickerDotView extends View implements ColorPickerView {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.M) {
            ...
            blackColor = getResources().getColor(R.color.draw_black);
        } else {
            whiteColor = getResources().getColor(R.color.draw_white, getContext().getTheme());
            blackColor = getResources().getColor(R.color.draw_black,getContext().getTheme());
        }
        ...
        } else if (circlePaint.getColor() == blackColor && ThemeUtils.isDarkTheme(getContext())) {
            canvas.drawCircle(getWidth() / 2, getHeight() / 2, dotRadius, eraserPaint);
            canvas.drawCircle(getWidth() / 2, getHeight() / 2, dotRadius - selectedRingSize, circlePaint);
        } else {
